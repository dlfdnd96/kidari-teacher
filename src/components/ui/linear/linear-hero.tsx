...
import { LinearH1, LinearText } from './linear-typography'
...
const ActionButton = memo(({ action, variant }: ActionButtonProps) => {
	...
	if (action.href && !isExternalLink) {
		...
		return (
			<LinearButton {...linkProps}>{sanitizeText(action.label)}</LinearButton>
		)
	}
	...
	return (
		<LinearButton {...baseProps}>{sanitizeText(action.label)}</LinearButton>
	)
})

ActionButton.displayName = 'ActionButton'

const LinearHero = forwardRef<HTMLElement, LinearHeroProps>(
	(
		{
			className,
...
LinearHero.displayName = 'LinearHero'
...
export { LinearHero, LinearSimpleHero, LinearFeatureHero }
