...
import VolunteerActivityEditForm from './VolunteerActivityEditForm'
...
const VolunteerActivityCard = memo(
	({
		...
		const isManager =
			currentUserId === activity.managerId || userRole === 'ADMIN'
		const canApply =
		...
		if (editing) {
			return (
				<VolunteerActivityEditForm
					...
					initialStartAt={new Date(activity.startAt)}
					initialEndAt={new Date(activity.endAt)}
					initialLocation={activity.location}
					initialApplicationDeadline={new Date(activity.applicationDeadline)}
					initialMaxParticipants={activity.maxParticipants || undefined}
					initialQualifications={activity.qualifications || undefined}
					initialMaterials={activity.materials || undefined}
		...
		return (
			...
			>
				...
				<div className="space-y-2 mb-4">
					...
					<div className="flex items-center gap-2 text-sm">
						...
						<span className="font-medium">
							{format(new Date(activity.startAt), 'M/d (E) HH:mm', {
								locale: ko,
							})}
							{activity.startAt !== activity.endAt && (
								<>
									{' '}
									~ {format(new Date(activity.endAt), 'HH:mm', { locale: ko })}
								</>
							)}
						</span>
					</div>
					...
					<div className="flex items-center gap-2 text-sm">
						...
						<span className="font-medium">
							{format(new Date(activity.applicationDeadline), 'M/d (E)', {
								locale: ko,
							})}
						</span>
						{new Date() > new Date(activity.applicationDeadline) && (
				...
				<div className="mt-4 flex flex-wrap gap-2">
					<button
						className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-semibold px-4 py-2 rounded-xl transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
						onClick={(e) => {
							e.stopPropagation()
							handleViewDetail()
						}}
